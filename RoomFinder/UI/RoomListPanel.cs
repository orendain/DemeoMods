...
namespace RoomFinder.UI {
  internal class RoomListPanel {
    ...
    public static RoomListPanel NewInstance() { return new RoomListPanel(); }

    private RoomListPanel() { panelObject = new GameObject("RoomListPanel"); }
    ...
    private void RenderRoomRow(RoomInfo room, int row) {
      ...
      roomRowContainer.transform.SetParent(panelObject.transform,
                                           worldPositionStays: false);
      ...
      var floorIndex =
          room.CustomProperties.TryGetValue("fi", out obj) ? (int)obj : -1;

      if (gameType == LevelSequence.GameType.Invalid || floorIndex < 0)
        return;

      var joinButton = UiUtil.CreateButton(JoinRoomAction(room.Name));
      joinButton.transform.SetParent(roomRowContainer.transform,
                                     worldPositionStays: false);
      joinButton.transform.localScale = new Vector3(0.4f, 0.7f, 0.7f);
      joinButton.transform.localPosition = new Vector3(-3f, 0, 0);

      var joinText = UiUtil.CreateText(room.Name, Color.white,
                                       UiUtil.DefaultLabelFontSize);
      joinText.transform.SetParent(roomRowContainer.transform,
                                   worldPositionStays: false);
      ...
      gameLabel.transform.SetParent(roomRowContainer.transform,
                                    worldPositionStays: false);
      gameLabel.transform.localPosition = new Vector3(-0.4f, 0, 0);

      var floorLabel = UiUtil.CreateLabelText(floorIndex.ToString());
      floorLabel.transform.SetParent(roomRowContainer.transform,
                                     worldPositionStays: false);
      floorLabel.transform.localPosition = new Vector3(1.75f, 0, 0);

      var playersText = $"{room.PlayerCount}/{room.MaxPlayers}";
      var playersLabel = UiUtil.CreateLabelText(playersText);
      playersLabel.transform.SetParent(roomRowContainer.transform,
                                       worldPositionStays: false);
