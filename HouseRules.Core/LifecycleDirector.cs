namespace HouseRules.Core
{
    ...
    internal static class LifecycleDirector
    {
        ...
        private static void CreatingGameState_TryCreateRoom_Prefix()
        {
            ...
            {
                CoreMod.Log.LogError("Failed to find required \"creatingGameState\" field.");
                return;
            }
        ...
        private static void AddModdedRoomProperties(RoomOptions roomOptions)
        {
            ...
            {
                CoreMod.Log.LogWarning($"Room options already include custom property: {ModdedRoomPropertyKey}");
                return;
            }
        ...
        private static void ActivateRuleset()
        {
            if (IsRulesetActive)
            {
                CoreMod.Log.LogWarning("Ruleset activation was attempted whilst a ruleset was already activated. This should not happen. Please report this to HouseRules developers.");
                return;
            }
            ...
            {
                CoreMod.Log.LogWarning($"The selected ruleset [{HR.SelectedRuleset.Name}] is not safe for multiplayer games. Skipping activation.");
                return;
            }

            IsRulesetActive = true;

            CoreMod.Log.LogInfo($"Activating ruleset: {HR.SelectedRuleset.Name} (with {HR.SelectedRuleset.Rules.Count} rules)");
            foreach (var rule in HR.SelectedRuleset.Rules)
            {
                try
                {
                    CoreMod.Log.LogDebug($"Activating rule type: {rule.GetType()}");
                    rule.OnActivate(_gameContext);
                }
                catch (Exception e)
                {
                    // TODO(orendain): Consider rolling back or disable rule.
                    CoreMod.Log.LogWarning($"Failed to activate rule [{rule.GetType()}]: {e}");
                }
            }
        }
        ...
        private static void DeactivateRuleset()
        {
            ...
            CoreMod.Log.LogInfo($"Deactivating ruleset: {HR.SelectedRuleset.Name} (with {HR.SelectedRuleset.Rules.Count} rules)");
            foreach (var rule in HR.SelectedRuleset.Rules)
            {
                try
                {
                    CoreMod.Log.LogDebug($"Deactivating rule type: {rule.GetType()}");
                    rule.OnDeactivate(_gameContext);
                }
                catch (Exception e)
                {
                    // TODO(orendain): Consider rolling back or disable rule.
                    CoreMod.Log.LogWarning($"Failed to deactivate rule [{rule.GetType()}]: {e}");
                }
            }
        }
        ...
        private static void OnPreGameCreated()
        {
            ...
            foreach (var rule in HR.SelectedRuleset.Rules)
            {
                try
                {
                    CoreMod.Log.LogDebug($"Calling OnPreGameCreated for rule type: {rule.GetType()}");
                    rule.OnPreGameCreated(_gameContext);
                }
                catch (Exception e)
                {
                    // TODO(orendain): Consider rolling back or disable rule.
                    CoreMod.Log.LogWarning($"Failed to successfully call OnPreGameCreated on rule [{rule.GetType()}]: {e}");
                }
            }
        }
        ...
        private static void OnPostGameCreated()
        {
            ...
            foreach (var rule in HR.SelectedRuleset.Rules)
            {
                try
                {
                    CoreMod.Log.LogDebug($"Calling OnPostGameCreated for rule type: {rule.GetType()}");
                    rule.OnPostGameCreated(_gameContext);
                }
                catch (Exception e)
                {
                    // TODO(orendain): Consider rolling back or disable rule.
                    CoreMod.Log.LogWarning($"Failed to successfully call OnPostGameCreated on rule [{rule.GetType()}]: {e}");
                }
            }
        }
