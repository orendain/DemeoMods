    ...
    public sealed class MonsterDeckOverriddenRule : Rule, IConfigWritable<MonsterDeckOverriddenRule.DeckConfig>, IPatchable, IMultiplayerSafe
    {
        ...
        private static bool AIDirectorController2_SpawnBossAndMinions_Prefix(ref AIDirectorContext context, ref TransientBoardState boardState, IRnd rng)
        {
            ...
            for (var i = list.Count - 1; i >= 0; i--)
            {
                ...
                {
                    EssentialsMod.Log.LogDebug(string.Format("Spawn zone doesn't contain any free tiles, removing it! {0}", spawnZone3.TileRect));
                    list.RemoveAt(i);
                }
            }
            ...
            for (var j = 0; j < list.Count; j++)
            {
                ...
                if (spawnZone4.NumStepsToExit >= AIDirectorConfig.KeyHolderMinDistanceToExit && spawnZone4.NumStepsToExit <= AIDirectorConfig.KeyHolderMaxDistanceToExit && spawnZone4.numStepsToEntrance >= AIDirectorConfig.KeyHolderMinDistanceToEntrance)
                {
                    ...
                    EssentialsMod.Log.LogDebug(string.Concat(new string[]
                    {
                        "Spawnzone had only ",
            ...
            if (spawnZone == null)
            {
                EssentialsMod.Log.LogWarning("MD: Could not find a spawn zone to spawn the boss");
            }
